...
cd "$($buildFolder)\..\"
mkdir gh-pages
git clone --quiet --branch=gh-pages https://$($username):$($personalAccessToken)@github.com/sankha93/ChakraCore.rs.git .\gh-pages\
cd gh-pages
git status
...
copy-item -path $($buildFolder)\target\doc\* -Destination $pwd.Path -Recurse
"<meta http-equiv=refresh content=0;url=numrs/index.html>" | Out-File index.html
...
if ($thereAreChanges -ne $null) { 
    ...
    git add --all
    git status
    git commit -m "Generate rustdocs"
    git status
    Write-Host "- Push it...."
    git push --quiet
